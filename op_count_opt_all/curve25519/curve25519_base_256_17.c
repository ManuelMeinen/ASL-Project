...
#include "../../base_256_17/256arithmetic/bignum.h"
...
void point_double_base_256_17(radix17_t *resX, radix17_t *resY, radix17_t *x, radix17_t *y)
{
    ...
    */
    ...
    radix17_t _2x, Axx, x2, _2x2, _3x2, _2y, l, l2, l3, tmp1, tmp2, one;
    // 2x = x+x
    add_256(&_2x, x, x);
    // Axx = A + 2x
    add_256(&Axx, &A_rad17, &_2x);
    // x2 = x^2
    square_256(&x2, x);
    // 2x^2 = x^2 + x^2
    add_256(&_2x2, &x2, &x2);
    ...
    // l = (_3x2 + A*_2x + 1)/(_2y)
    mul_256(&tmp1, &A_rad17, &_2x);
    add_256(&tmp1, &tmp1, &_3x2);
    add_256(&tmp1, &tmp1, &const_1_rad17);
    div_256(&l, &tmp1, &_2y);
    // l2 = l^2
    square_256(&l2, &l);
    // l3 = l2*l
...
void point_add_base_256_17(radix17_t *resX, radix17_t *resY, radix17_t *op1x, radix17_t *op1y, radix17_t *op2x, radix17_t *op2y)
{
    ...
    radix17_t diffx, diffy, frac, frac2, frac3, Axx, tmp1, tmp2;
    // diffx = x2-x1
