...
void point_double_base_256_25(__m256i *resXlo, __m256i *resXhi, __m256i *resYlo,  __m256i *resYhi, __m256i xlo, __m256i xhi, __m256i ylo, __m256i yhi)
{
    /*
    res = 2*(x,y)
    */
    ...
    // Axx = A + 2x
    add_256(&Axxlo, &Axxhi, A_rad25lo, A_rad25hi, _2xlo, _2xhi);
    // x2 = x^2
    ...
    // l = (_3x2 + A*_2x + 1)/(_2y)
    mul_256(&tmp1lo, &tmp1hi, A_rad25lo, A_rad25hi, _2xlo, _2xhi);
    add_256(&tmp1lo, &tmp1hi, tmp1lo, tmp1hi, _3x2lo, _3x2hi);
    add_256(&tmp1lo, &tmp1hi, tmp1lo, tmp1hi, const_1_rad25lo, const_1_rad25hi);
...
void point_add_base_256_25(__m256i *resXlo, __m256i *resXhi, __m256i *resYlo, __m256i *resYhi, __m256i op1xlo, __m256i op1xhi, __m256i op1ylo, __m256i op1yhi, __m256i op2xlo, __m256i op2xhi, __m256i op2ylo, __m256i op2yhi)
{
    // resX <- op1x + op2x
    // resY <- op1y + op2y
    ...
    {
        // point_add_base_256_25(p1, p1) = double(p1), otherwise exception div by 0
        point_double_base_256_25(resXlo, resXhi, resYlo, resYhi, op1xlo, op1xhi, op1ylo, op1yhi);
        return;
    }
    ...
    // Axx = A + x1 + x2
    add_256(&tmp1lo, &tmp1hi, A_rad25lo, A_rad25hi, op1xlo, op1xhi);
...
void scalar_mult_opt_all_25(__m256i base_point_xlo, __m256i base_point_xhi, __m256i base_point_ylo, __m256i base_point_yhi, char *sk, __m256i *Pkxlo, __m256i *Pkxhi, __m256i *Pkylo, __m256i *Pkyhi)
{
    ...
    */
    ...
    for(int i=0; i<n; i++){
        ...
        if(sk[i]=='0'){
            // R1 <- add (R0, R1)
            point_add_base_256_25(&R1xlo, &R1xhi, &R1ylo, &R1yhi, R0xlo, R0xhi, R0ylo, R0yhi, R1xlo, R1xhi, R1ylo, R1yhi);
            //copy_256(&R1xlo, &R1xhi, resXlo, resXhi);
            //copy_256(&R1ylo, &R1yhi, resYlo, resYhi);
            // R0 <- double(R0)
            point_double_base_256_25(&R0xlo, &R0xhi, &R0ylo, &R0yhi, R0xlo, R0xhi, R0ylo, R0yhi);
            // copy_256(&R0xlo, &R0xhi, resXlo, resXhi);
            // copy_256(&R0ylo, &R0yhi, resYlo, resYhi);
                
        }else{
            // R0 <- add(R0, R1)
            point_add_base_256_25(&R0xlo, &R0xhi, &R0ylo, &R0yhi, R0xlo, R0xhi, R0ylo, R0yhi, R1xlo, R1xhi, R1ylo, R1yhi);
            // copy_256(&R0xlo, &R0xhi, resXlo, resXhi);
            // copy_256(&R0ylo, &R0yhi, resYlo, resYhi);
            // R1 <- double(R1)
            point_double_base_256_25(&R1xlo, &R1xhi, &R1ylo, &R1yhi, R1xlo, R1xhi, R1ylo, R1yhi);
            // copy_256(&R1xlo, &R1xhi, resXlo, resXhi);
            // copy_256(&R1ylo, &R1yhi, resYlo, resYhi);
        }
    }
...
void public_key_gen_opt_all_25(char *sk, __m256i *xlo, __m256i *xhi, __m256i *ylo, __m256i *yhi)
{
    ...
    (x,y) = sk*G
    */
    scalar_mult_opt_all_25(Gx_rad25lo, Gx_rad25hi, Gy_rad25lo, Gy_rad25hi, sk, xlo, xhi, ylo, yhi);
}
...
int validate_opt_all_25(char **sk_A, char **sk_B, int num_test_cases) {
    ...
    */
	...
	for ( int i = 0; i < num_test_cases; ++i ) {
		...
		public_key_gen_opt_all_25(sk_A[i], &Pkx_Alo, &Pkx_Ahi, &Pky_Alo, &Pky_Ahi);
    	// 2. generate Bob's public key
		public_key_gen_opt_all_25(sk_B[i], &Pkx_Blo, &Pkx_Bhi, &Pky_Blo, &Pky_Bhi);
		// 3. Generate the shared secret
		scalar_mult_opt_all_25(Pkx_Blo, Pkx_Bhi, Pky_Blo, Pky_Bhi, sk_A[i], &ssx_Alo, &ssx_Ahi, &ssy_Alo, &ssy_Ahi);
		scalar_mult_opt_all_25(Pkx_Alo, Pkx_Ahi, Pky_Alo, Pky_Ahi, sk_B[i], &ssx_Blo, &ssx_Bhi, &ssy_Blo, &ssy_Bhi);
...
int checkPk_opt_all_25(char **sk, char **pk, int num_test_cases) {
    ...
    */
	...
	for ( int i = 0; i < num_test_cases; ++i ) {
		public_key_gen_opt_all_25(sk[i], &Pkxlo, &Pkxhi, &Pkylo, &Pkyhi);
		radix25_vec_toString( pkAsString, Pkxlo, Pkxhi );
