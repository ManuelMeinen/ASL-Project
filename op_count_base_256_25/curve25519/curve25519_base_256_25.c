...
void point_double_base_256_25(radix25_t *resX, radix25_t *resY, radix25_t *x, radix25_t *y)
{
    /*
    res = 2*(x,y)
    */
    ...
    radix25_t upperfrac, lowerfrac, tmp1, tmp2, tmp3, l;
    ...
    //x - coordinate
    mul_256(&tmp1, &A_rad25, x);               // tmp1 = A*x
    add_256(&tmp1, &tmp1, &tmp1);          // tmp1 = 2*tmp1
    add_256(&tmp1, &tmp1, &const_1_rad25);       // tmp1 = tmp1 + 1 

    square_256(&tmp2, x);                // tmp2 = x^2
    mul_256(&tmp2, &tmp2, &const_3_rad25);       // tmp2 = tmp2*3

    add_256(&upperfrac, &tmp2, &tmp1);     // upperfrac = tmp2+tmp1

    mul_256(&lowerfrac, &B_rad25, y);          // lowerfrac = B*y
    ...
    mul_256(resX, &B_rad25, &l);           // resX = B*l
    sub_256(resX, resX, &A_rad25);    // resX = resX - A
    ...
    mul_256(&tmp1, x, &const_3_rad25);           // tmp1 = 3*x
    add_256(&tmp1, &tmp1, &A_rad25);                // tmp1 = tmp1+A
...
void point_add_base_256_25(radix25_t *resX, radix25_t *resY, radix25_t *op1x, radix25_t *op1y, radix25_t *op2x, radix25_t *op2y)
{    
    // resX <- op1x + op2x
    // resY <- op1y + op2y
    ...
    if ( cmp_point_256( op1x, op1y, op2x, op2y ) ) { 
    	...
    	point_double_base_256_25( resX, resY, op1x, op1y );
    	return;
    }

    radix25_t diffx, diffy, tmp1, tmp2;
    set_ui_256( &diffx   , 0 ); 
    set_ui_256( &diffy   , 0 );
    set_ui_256( &tmp1    , 0 );
    set_ui_256( &tmp2    , 0 );
    ...
    mul_256( &diffy, &diffy, &B_rad25 );			// diffy = B * diffy
    div_256( resX, &diffy, &diffx );		// resX = diffy / diffx
    
    sub_256( resX, resX, &A_rad25 );		// resX -= A
	...
	add_256( &tmp1, &tmp1, &A_rad25   );			// tmp1 = tmp1 + A, i.e. tmp1 = 2*x1 + x2 + A
	...
	mul_256   ( &diffy, &diffy, &B_rad25 );			// diffy = diffy * B
...
void scalar_mult_base_256_25(radix25_t *base_point_x, radix25_t *base_point_y, char *sk, radix25_t *Pkx, radix25_t *Pky)
{
    ...
    */
    int n = sk_length;
    radix25_t R0x, R0y, R1x, R1y, resX, resY;
    set_ui_256(&R0x, 0);
    set_ui_256(&R0y, 0);
    ...
    for(int i=0; i<n; i++){
        ...
        if(sk[i]=='0'){
            // R1 <- add (R0, R1)
            point_add_base_256_25(&resX, &resY, &R0x, &R0y, &R1x, &R1y);
            copy_256(&R1x, resX);
            copy_256(&R1y, resY);
            ...
            set_ui_256(&resY, 0);
            // R0 <- double(R0)
            point_double_base_256_25(&resX, &resY, &R0x, &R0y);
            copy_256(&R0x, resX);
            copy_256(&R0y, resY);
        }else{
            // R0 <- add(R0, R1)
            ...
            set_ui_256(&resY, 0);
            // R1 <- double(R1)
            point_double_base_256_25(&resX, &resY, &R1x, &R1y);
            copy_256(&R1x, resX);
            copy_256(&R1y, resY);
        }
    }
...
void public_key_gen_base_256_25(char *sk, radix25_t *x, radix25_t *y)
{
    ...
    (x,y) = sk*G
    */
    scalar_mult_base_256_25(&Gx_rad25, &Gy_rad25, sk, x, y);
    /*
...
int validate_base_256_25(char **sk_A, char **sk_B, int num_test_cases) {
	...
	radix25_t Pkx_A, Pky_A, Pkx_B, Pky_B, ssx_A, ssy_A, ssx_B, ssy_B;
	set_ui_256( &Pkx_A, 0 );
	set_ui_256( &Pky_A, 0 );
	set_ui_256( &Pkx_B, 0 );
	set_ui_256( &Pky_B, 0 );
	...
	for ( int i = 0; i < num_test_cases; ++i ) {
		...
		public_key_gen_base_256_25(sk_A[i], &Pkx_A, &Pky_A);
    	// 2. generate Bob's public key
		public_key_gen_base_256_25(sk_B[i], &Pkx_B, &Pky_B);
		// 3. Generate the shared secret
		scalar_mult_base_256_25(&Pkx_B, &Pky_B, sk_A[i], &ssx_A, &ssy_A);
		scalar_mult_base_256_25(&Pkx_A, &Pky_A, sk_B[i], &ssx_B, &ssy_B);
...
int checkPk_base_256_25(char **sk, char **pk, int num_test_cases) {
	...
	for ( int i = 0; i < num_test_cases; ++i ) {
		public_key_gen_base_256_25(sk[i], &Pkx, &Pky);
		radix25_toString( pkAsString, Pkx );
