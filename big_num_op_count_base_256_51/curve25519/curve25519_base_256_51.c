...
void point_double(radix51_t *resX, radix51_t *resY, radix51_t *x, radix51_t *y)
{
    /*
    res = 2*(x,y)
    */
    ...
    radix51_t upperfrac, lowerfrac, tmp1, tmp2, tmp3, l;
    ...
    //x - coordinate
    mul_256(&tmp1, &A_rad51, x);               // tmp1 = A*x
    add_256(&tmp1, &tmp1, &tmp1);          // tmp1 = 2*tmp1
    add_256(&tmp1, &tmp1, &const_1_rad51);       // tmp1 = tmp1 + 1 

    square_256(&tmp2, x);                // tmp2 = x^2
    mul_256(&tmp2, &tmp2, &const_3_rad51);       // tmp2 = tmp2*3

    add_256(&upperfrac, &tmp2, &tmp1);     // upperfrac = tmp2+tmp1

    mul_256(&lowerfrac, &B_rad51, y);          // lowerfrac = B*y
    ...
    mul_256(resX, &B_rad51, &l);           // resX = B*l
    sub_256(resX, resX, &A_rad51);    // resX = resX - A
    ...
    mul_256(&tmp1, x, &const_3_rad51);           // tmp1 = 3*x
    add_256(&tmp1, &tmp1, &A_rad51);                // tmp1 = tmp1+A
...
void point_add(radix51_t *resX, radix51_t *resY, radix51_t *op1x, radix51_t *op1y, radix51_t *op2x, radix51_t *op2y)
{    
    // resX <- op1x + op2x
    // resY <- op1y + op2y
    ...
    if ( cmp_point_256( op1x, op1y, op2x, op2y ) ) { 
    	...
    	point_double( resX, resY, op1x, op1y );
    	return;
    }

    radix51_t diffx, diffy, tmp1, tmp2;
    set_ui_256( &diffx   , 0 ); 
    set_ui_256( &diffy   , 0 );
    set_ui_256( &tmp1    , 0 );
    set_ui_256( &tmp2    , 0 );
    ...
    mul_256( &diffy, &diffy, &B_rad51 );			// diffy = B * diffy
    div_256( resX, &diffy, &diffx );		// resX = diffy / diffx
    
    sub_256( resX, resX, &A_rad51 );		// resX -= A
	...
	add_256( &tmp1, &tmp1, &A_rad51 );			// tmp1 = tmp1 + A, i.e. tmp1 = 2*x1 + x2 + A
	...
	mul_256   ( &diffy, &diffy, &B_rad51 );			// diffy = diffy * B
...
void scalar_mult(radix51_t *base_point_x, radix51_t *base_point_y, char *sk, radix51_t *Pkx, radix51_t *Pky)
{
    ...
    */
    ...
    for(int i=0; i<n; i++){
        ...
        if(sk[i]=='0'){
            // R1 <- add (R0, R1)
            point_add(&resX, &resY, &R0x, &R0y, &R1x, &R1y);
            copy_256(&R1x, resX);
            copy_256(&R1y, resY);
            ...
            set_ui_256(&resY, 0);
            // R0 <- double(R0)
            point_double(&resX, &resY, &R0x, &R0y);
            copy_256(&R0x, resX);
            copy_256(&R0y, resY);
        }else{
            // R0 <- add(R0, R1)
            ...
            set_ui_256(&resY, 0);
            // R1 <- double(R1)
            point_double(&resX, &resY, &R1x, &R1y);
            copy_256(&R1x, resX);
            copy_256(&R1y, resY);
        }
    }
    ...
    printf("big number ops %d: ", get_bignum_ops());
}
...
void pub_key_gen(char *sk, radix51_t *x, radix51_t *y)
{
    ...
    (x,y) = sk*G
    */
    scalar_mult(&Gx_rad51, &Gy_rad51, sk, x, y);
    /*
    print_256(x, "Pk(x)");
    print_256(y, "Pk(y)");
    */                        
...
int validate(char **sk_A, char **sk_B, int num_test_cases) {
    ...
    */
	...
	for ( int i = 0; i < num_test_cases; ++i ) {
		...
		pub_key_gen(sk_A[i], &Pkx_A, &Pky_A);
    	// 2. generate Bob's public key
		pub_key_gen(sk_B[i], &Pkx_B, &Pky_B);
		// 3. Generate the shared secret
		scalar_mult(&Pkx_B, &Pky_B, sk_A[i], &ssx_A, &ssy_A);
		scalar_mult(&Pkx_A, &Pky_A, sk_B[i], &ssx_B, &ssy_B);
...
int checkPk(char **sk, char **pk, int num_test_cases) {
    /*
