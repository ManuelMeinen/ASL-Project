#include <stdio.h>
#include "256_17_vec_naive_bignum.h"
#include <immintrin.h>
#include <emmintrin.h>
#include "../../_common_libs/radix17.h"
#include "vec_constants.h"

const int NO_LIMBS_VEC_N = 15;
const int LIMB_SIZE_VEC_N = 17;

void copy_256_17_vecn(radix17_t *dest, radix17_t src)
{
    //set dest to src

    for (size_t i = 0; i < NO_LIMBS_VEC_N; i++)
    {
        dest->d[i] = src.d[i];
    }
}
void reduce_256_17_vecn(radix17_t *x)
{
    ...
    carry = d0 >> LIMB_SIZE_VEC_N;
    d1 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d0 -= carry;

    carry = d1 >> LIMB_SIZE_VEC_N;
    d2 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d1 -= carry;

    carry = d2 >> LIMB_SIZE_VEC_N;
    d3 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d2 -= carry;

    carry = d3 >> LIMB_SIZE_VEC_N;
    d4 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d3 -= carry;

    carry = d4 >> LIMB_SIZE_VEC_N;
    d5 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d4 -= carry;

    carry = d5 >> LIMB_SIZE_VEC_N;
    d6 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d5 -= carry;

    carry = d6 >> LIMB_SIZE_VEC_N;
    d7 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d6 -= carry;

    carry = d7 >> LIMB_SIZE_VEC_N;
    d8 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d7 -= carry;

    carry = d8 >> LIMB_SIZE_VEC_N;
    d9 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d8 -= carry;

    carry = d9 >> LIMB_SIZE_VEC_N;
    d10 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d9 -= carry;

    carry = d10 >> LIMB_SIZE_VEC_N;
    d11 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d10 -= carry;

    carry = d11 >> LIMB_SIZE_VEC_N;
    d12 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d11 -= carry;

    carry = d12 >> LIMB_SIZE_VEC_N;
    d13 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d12 -= carry;

    carry = d13 >> LIMB_SIZE_VEC_N;
    d14 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    d13 -= carry;

    carry = d14 >> LIMB_SIZE_VEC_N;
    d0 += 19 * carry;
    carry <<= LIMB_SIZE_VEC_N;
    d14 -= carry;
...
void reduce_mul_256_17_vecn(unsigned long r[30])
{
    ...
    carry = r0 >> LIMB_SIZE_VEC_N;
    r1 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r0 -= carry;

    carry = r1 >> LIMB_SIZE_VEC_N;
    r2 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r1 -= carry;

    carry = r2 >> LIMB_SIZE_VEC_N;
    r3 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r2 -= carry;

    carry = r3 >> LIMB_SIZE_VEC_N;
    r4 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r3 -= carry;

    carry = r4 >> LIMB_SIZE_VEC_N;
    r5 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r4 -= carry;

    carry = r5 >> LIMB_SIZE_VEC_N;
    r6 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r5 -= carry;

    carry = r6 >> LIMB_SIZE_VEC_N;
    r7 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r6 -= carry;

    carry = r7 >> LIMB_SIZE_VEC_N;
    r8 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r7 -= carry;

    carry = r8 >> LIMB_SIZE_VEC_N;
    r9 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r8 -= carry;

    carry = r9 >> LIMB_SIZE_VEC_N;
    r10 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r9 -= carry;

    carry = r10 >> LIMB_SIZE_VEC_N;
    r11 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r10 -= carry;

    carry = r11 >> LIMB_SIZE_VEC_N;
    r12 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r11 -= carry;

    carry = r12 >> LIMB_SIZE_VEC_N;
    r13 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r12 -= carry;

    carry = r13 >> LIMB_SIZE_VEC_N;
    r14 += carry;
    carry <<= LIMB_SIZE_VEC_N;
    r13 -= carry;

    carry = r14 >> LIMB_SIZE_VEC_N;
    r0 += 19 * carry;
    carry <<= LIMB_SIZE_VEC_N;
    r14 -= carry;
...
void reduce_mul_512_17_vecn(unsigned long r[30])
{
    // backup r[14]
    long r14 = r[14];
...
void mul_256_17_vecn(radix17_t *res, radix17_t *x, radix17_t *y)
{
	
    unsigned long r[NO_LIMBS_VEC_N*2];
	/*
	//non-vectorized code as reference
	
    //unsigned long r[NO_LIMBS_VEC_N*2 - 1];
	...
	*/
    ...
    reduce_mul_512_17_vecn(r);

    reduce_mul_256_17_vecn(r);
    
    for(int i = 0; i<NO_LIMBS_VEC_N; i++){
        res->d[i] = r[i];
    }
...
int cmp_coord_256_17_vecn(radix17_t *x1, radix17_t *x2)
{
    ...
    for (size_t i = 1; i < NO_LIMBS_VEC_N; i++)
    {
...
int cmp_point_256_17_vecn(radix17_t *x1, radix17_t *y1, radix17_t *x2, radix17_t *y2)
{
    ...
    return cmp_coord_256_17_vecn(x1, y1) && cmp_coord_256_17_vecn(x2, y2);
}
...
void inverse_256_17_vecn(radix17_t *res, radix17_t *x)
{
    ...
    int i;
    /* 2 */ square_256_17_vecn(&z2, x);
    /* 4 */ square_256_17_vecn(&t, &z2);
    /* 8 */ square_256_17_vecn(&t, &t);
    /* 9 */ mul_256_17_vecn(&z9, &t, x);
    /* 11 */ mul_256_17_vecn(&z11, &z9, &z2);
    /* 22 */ square_256_17_vecn(&t, &z11);
    /* 2^5 - 2^0 = 31 */ mul_256_17_vecn(&z2_5_0, &t, &z9);
    /* 2^6 - 2^1 */ square_256_17_vecn(&t, &z2_5_0);
    /* 2^10 - 2^5 */ for (i = 1; i < 5; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^10 - 2^0 */ mul_256_17_vecn(&z2_10_0, &t, &z2_5_0);
    /* 2^11 - 2^1 */ square_256_17_vecn(&t, &z2_10_0);
    /* 2^20 - 2^10 */ for (i = 1; i < 10; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^20 - 2^0 */ mul_256_17_vecn(&z2_20_0, &t, &z2_10_0);
    /* 2^21 - 2^1 */ square_256_17_vecn(&t, &z2_20_0);
    /* 2^40 - 2^20 */ for (i = 1; i < 20; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^40 - 2^0 */ mul_256_17_vecn(&t, &t, &z2_20_0);
    /* 2^41 - 2^1 */ square_256_17_vecn(&t, &t);
    /* 2^50 - 2^10 */ for (i = 1; i < 10; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^50 - 2^0 */ mul_256_17_vecn(&z2_50_0, &t, &z2_10_0);
    /* 2^51 - 2^1 */ square_256_17_vecn(&t, &z2_50_0);
    /* 2^100 - 2^50 */ for (i = 1; i < 50; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^100 - 2^0 */ mul_256_17_vecn(&z2_100_0, &t, &z2_50_0);
    /* 2^101 - 2^1 */ square_256_17_vecn(&t, &z2_100_0);
    /* 2^200 - 2^100 */ for (i = 1; i < 100; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^200 - 2^0 */ mul_256_17_vecn(&t, &t, &z2_100_0);
    /* 2^201 - 2^1 */ square_256_17_vecn(&t, &t);
    /* 2^250 - 2^50 */ for (i = 1; i < 50; i++)
    {
        square_256_17_vecn(&t, &t);
    }
    /* 2^250 - 2^0 */ mul_256_17_vecn(&t, &t, &z2_50_0);
    /* 2^251 - 2^1 */ square_256_17_vecn(&t, &t);
    /* 2^252 - 2^2 */ square_256_17_vecn(&t, &t);
    /* 2^253 - 2^3 */ square_256_17_vecn(&t, &t);
    /* 2^254 - 2^4 */ square_256_17_vecn(&t, &t);
    /* 2^255 - 2^5 */ square_256_17_vecn(&t, &t);
    /* 2^255 - 21 */ mul_256_17_vecn(res, &t, &z11);
...
void div_256_17_vecn(radix17_t *res, radix17_t *a, radix17_t *b)
{
    // inverse and mult
    radix17_t inv;
    inverse_256_17_vecn(&inv, b);
    mul_256_17_vecn(res, a, &inv);
}